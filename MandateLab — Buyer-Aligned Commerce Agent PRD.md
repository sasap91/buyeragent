...
Commerce execution: finding and attempting to purchase a product.
Mandate authorization: independently determining whether the proposed transaction is permissible.
...
MandateLab should:
Build or retrieve the Buyer Preference Profile.
Parse the current request into:
Hard constraints
Soft preferences
Authorization rules
Material ambiguities
Search a controlled product catalog.
Evaluate each candidate against the hard constraints.
Remove products that fail a hard constraint.
Identify meaningful trade-offs among the remaining products.
Rank permissible products using the buyer’s preferences.
Select the best option or ask one targeted question when uncertainty materially affects the decision.
Return APPROVE, REVIEW, or BLOCK.
Replan when a candidate is rejected and another permissible option may exist.
Refresh and revalidate the final cart immediately before checkout.
Complete a test or sandbox transaction only when authorized.
Store the mission outcome and buyer feedback as learning-ready events.
...
Two buyers receive different recommendations from the same catalog.
Current explicit intent overrides historical preferences.
An attractive product is blocked because it violates a hard constraint.
A permissible product returns REVIEW because it exceeds autonomous authority.
Missing condition or delivery information prevents autonomous execution.
A rejected product produces a successful replanning attempt.
A final price, delivery, or product change is detected before checkout.
A valid and authorized sandbox transaction completes successfully.
...
Freeze the four shared data contracts.
Create the normalized product catalog and buyer fixtures.
Build deterministic hard-constraint validation.
Build both preference-profile generators in parallel.
Add lightweight buyer-specific ranking.
Implement APPROVE, REVIEW, and BLOCK.
Connect the controlled search and sandbox cart.
Add final pre-checkout revalidation.
Add one rejection-and-replanning flow.
Add explanations and event logging.
...
13. Demo Flow
Show two buyers with different preference profiles.
Give both buyers the same product catalog.
Enter a current purchase request.
Show the generated mandate.
Show an attractive product rejected by a hard constraint.
Show the agent replan using the rejection reason.
Show different best products for the two buyers.
Show one transaction requiring human approval.
Change the final price or delivery information.
Demonstrate final validation catching the change.
Complete one authorized sandbox purchase.
Show the recorded trajectory and buyer feedback.
...
Owner   Required before live demo
Luke    Implement the shared PreferenceProfileBuilder output. His package currently   does not return BuyerPreferenceProfile.
Luke    Provide the referenced user_profile.server implementation or a FastAPI router for /api/update; that module is currently missing.
Sasa    Confirm a live headphones-category profile path. The current history demo primarily uses grocery/coffee data; Theo’s headphones profile is presently a fixture.
Shared Connect Luke’s frontend to the new MandateLab API and decide how Sasa’s trajectory logger consumes shared decisions/outcomes.


One practical conflict also needs agreement: Luke’s frontend proxies /api to port 8000, which is now the MandateLab API port. The clean solution is for Luke to expose an APIRouter that can be mounted into mandatelab_api, rather than starting a second server on the same port.
